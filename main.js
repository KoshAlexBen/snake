...
function initContent() {
  ...
  // Определяем, действительно ли мы внутри Telegram Mini App:
  // наличие initDataUnsafe с каким‑то содержимым означает запуск из Telegram,
  // а просто наличие объекта Telegram.WebApp в браузере — нет.
  const tg = window.Telegram && window.Telegram.WebApp;
  const hasInitData =
    !!tg && !!tg.initDataUnsafe && Object.keys(tg.initDataUnsafe).length > 0;
  const isInTelegram = !!tg && hasInitData;
